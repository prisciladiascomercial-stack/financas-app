...
import { fmt, fmtDateShort, MESES, CATS_REC, todayISO } from '@/lib/utils'
import { csvReceitas } from '@/lib/export'
import StatusBadge from './StatusBadge'

export default function TabReceitas({ receitas, mes, ano, config, logoDataUrl, addReceita, updateReceita, deleteReceita, setShowAI }: any) {
      ...
      <div className="card">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={()=>csvReceitas(receitas,mes,ano)} className="btn-outline text-xs">📊 CSV</button>
        ...
        {showForm&&(
          <div className="bg-blue-50 rounded-xl p-3 mb-3 space-y-2">
            <div className="grid grid-cols-2 gap-2">
              ...
              <div className="col-span-2"><label className="text-xs text-gray-500">Descrição *</label><input className="input text-sm" placeholder="Ex: Faturamento clientes" value={form.descricao} onChange={e=>sf('descricao',e.target.value)}/></div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead><tr>
              ...
              <th className="table-th" style={{width:180}}>Status / Ações</th>
            </tr></thead>
            <tbody>
              {receitas.length===0&&<tr><td colSpan={5} className="text-center text-gray-400 text-sm py-8">Nenhuma receita lançada</td></tr>}
              {receitas.map((r:any)=>{
                if(editing===r.id) return (
                  <tr key={r.id} className="bg-blue-50">
                    ...
                    <td className="px-2 py-1 whitespace-nowrap">
                      <button onClick={()=>{updateReceita(r.id,{...draft,valor:parseFloat(draft.valor)});setEditing(null)}} className="text-xs px-2 py-1 bg-green-600 text-white rounded mr-1">✓</button>
                    ...
                    <td className="table-td whitespace-nowrap">
                      <div className="flex items-center gap-1.5">
                        ...
                        <div className="opacity-0 group-hover:opacity-100 flex gap-1">
                          ...
                          <button onClick={()=>deleteReceita(r.id)} className="text-xs px-2 py-0.5 border border-red-100 rounded text-red-400 hover:bg-red-50">×</button>
                        </div>
                      </div>
                    </td>
                  </tr>
