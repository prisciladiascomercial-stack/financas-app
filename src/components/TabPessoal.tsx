...
import { exportarCSVPessoal, exportarPDFPorCategoria } from '@/lib/export'
      ...
      <table className="w-full">
        <thead>
          <tr>
            ...
            <th className="table-th" style={{ width: 80 }}>Cartao</th>
            <th className="table-th" style={{ width: 110 }}>Valor</th>
            <th className="table-th" style={{ width: 150 }}>Status / Acoes</th>
            <th className="table-th" style={{ width: 130 }}>Pago em</th>
          </tr>
        </thead>
        <tbody>
          {items.length === 0 && <tr><td colSpan={7} className="text-center text-gray-400 text-sm py-6">Nenhum lancamento</td></tr>}
          {items.map((r: any) => {
            if (editing === r.id) {
              return (
                <tr key={r.id} className="bg-blue-50">
                  ...
                  <td className="px-2 py-1"><input className="input text-xs py-1" placeholder="Cartao" value={draft.cartao || ''} onChange={e => setDraft((d: any) => ({ ...d, cartao: e.target.value }))} /></td>
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full">
        ...
        <tbody>
          {items.length === 0 && <tr><td colSpan={5} className="text-center text-gray-400 text-sm py-6">Nenhuma pendencia</td></tr>}
          {items.map((r: any) => {
            if (editing === r.id) {
              return (
                <tr key={r.id} className="bg-blue-50">
...
export default function TabPessoal({ pessoal, pendencias, mes, ano, config, logoDataUrl, addPessoal, updatePessoal, deletePessoal, baixarPessoal, addPendencia, updatePendencia, deletePendencia, transportarPendentes, setShowAI }: any) {
  ...
  async function add() {
    ...
    await addPessoal({ ...form, valor: parseFloat(form.valor), pago: false })
    setForm({ vencimento: '', descricao: '', categoria: 'Moradia', valor: '', cartao: '', observacao: '' })
    setShowForm(false)
  }

  async function addP() {
    if (!pform.descricao || !pform.valor) return
    await addPendencia({ ...pform, valor: parseFloat(pform.valor), pago: false, origem: 'pessoal' })
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="card">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button onClick={() => exportarCSVPessoal(pessoal, mes, ano)} className="btn-outline text-xs">CSV</button>
            <button onClick={() => exportarPDFPorCategoria(pessoal, 'pessoal', mes, ano, config, logoDataUrl)} className="btn-outline text-xs">PDF</button>
        ...
        {showForm && (
          <div className="bg-blue-50 rounded-xl p-3 mb-3 space-y-2">
            <div className="grid grid-cols-2 gap-2">
              ...
              <div className="col-span-2"><label className="text-xs text-gray-500">Descricao *</label><input className="input text-sm" placeholder="Ex: Condominio" value={form.descricao} onChange={e => sf('descricao', e.target.value)} /></div>
              ...
              <div><label className="text-xs text-gray-500">Cartao</label><input className="input text-sm" placeholder="Ex: Nubank" value={form.cartao} onChange={e => sf('cartao', e.target.value)} /></div>
              <div className="col-span-2"><label className="text-xs text-gray-500">Observacao</label><input className="input text-sm" value={form.observacao} onChange={e => sf('observacao', e.target.value)} /></div>
            </div>
      ...
      <div className="card">
        <div className="flex items-center justify-between mb-3">
          <div>
            <h3 className="text-sm font-semibold">Pendencias de meses anteriores</h3>
      ...
      {showImport && <ImportModal targetTab="pessoal" onImport={handleImport} onClose={() => setShowImport(false)} />}
    </div>
  )
}
