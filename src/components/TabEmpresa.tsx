...
import { csvEmpresa, pdfCategoria } from '@/lib/export'
    ...
    <div className="overflow-x-auto">
      <table className="w-full">
        <thead><tr>
          ...
          <th className="table-th" style={{width:200}}>Status / Ações</th>
        </tr></thead>
        <tbody>
          {items.length===0&&<tr><td colSpan={5} className="text-center text-gray-400 text-sm py-6">Nenhum lançamento</td></tr>}
          {items.map((r:any)=>{
            ...
            return (
              <tr key={r.id} className="hover:bg-gray-50 group">
                ...
                <td className="table-td max-w-0"><span title={r.descricao} className="truncate block">{r.descricao}{r.transportado&&<span className="ml-1 text-xs bg-blue-100 text-blue-700 px-1 rounded">transp.</span>}</span>{r.observacao&&<span className="text-xs text-gray-400 italic"> — {r.observacao}</span>}</td>
                ...
                <td className="table-td whitespace-nowrap">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    ...
                    {r.pago_em&&<span className="text-xs text-gray-400">{fmtDateShort(r.pago_em)}</span>}
...
export default function TabEmpresa({ empresa, pendencias, mes, ano, config, logoDataUrl, addEmpresa, updateEmpresa, deleteEmpresa, baixarEmpresa, addPendencia, updatePendencia, deletePendencia, transportarPendentes, setShowAI }: any) {
  ...
  const [form, setForm] = useState({vencimento:'',descricao:'',categoria:'Funcionário',valor:'',observacao:''})
  ...
  async function add(){if(!form.descricao||!form.valor)return;await addEmpresa({...form,valor:parseFloat(form.valor),pago:false});setForm({vencimento:'',descricao:'',categoria:'Funcionário',valor:'',observacao:''});setShowForm(false)}
  async function addP(){if(!pform.descricao||!pform.valor)return;await addPendencia({...pform,valor:parseFloat(pform.valor),pago:false,origem:'empresa'});setPform({vencimento:'',descricao:'',valor:'',observacao:''});setShowPF(false)}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="card">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button onClick={()=>csvEmpresa(empresa,mes,ano)} className="btn-outline text-xs">📊 CSV</button>
            <button onClick={()=>pdfCategoria(empresa,'Empresa',mes,ano,config,logoDataUrl)} className="btn-outline text-xs">📄 PDF</button>
        ...
        {showForm&&(
          <div className="bg-blue-50 rounded-xl p-3 mb-3 space-y-2">
            <div className="grid grid-cols-2 gap-2">
              ...
              <div className="col-span-2"><label className="text-xs text-gray-500">Descrição *</label><input className="input text-sm" placeholder="Ex: Sr Valter" value={form.descricao} onChange={e=>sf('descricao',e.target.value)}/></div>
              ...
              <div><label className="text-xs text-gray-500">Observação</label><input className="input text-sm" value={form.observacao} onChange={e=>sf('observacao',e.target.value)}/></div>
            </div>
      ...
      <div className="card">
        <div className="flex items-center justify-between mb-3">
          <div><h3 className="text-sm font-semibold">Pendências de meses anteriores</h3>{totalPend>0&&<p className="text-xs text-red-500">Em aberto: {fmt(totalPend)}</p>}</div>
        ...
        <TabelaDespesas items={pendEmp} cats={CATS_EMP} onUpdate={updatePendencia} onDelete={deletePendencia} onBaixar={(_id:string,pago:boolean)=>updatePendencia(_id,{pago:!pago,pago_em:!pago?todayISO():null})}/>
      </div>
    </div>
  )
}
